...
class StarredMessage:
    def __init__(self, bot, *, message_id, channel_id, guild_id, stars=0, sent_msg_id):
        self.bot = bot
    ...
    def __repr__(self):
        return (
            "<{0.__class__.__name__} message_id={0.message_id} "
    ...
    async def update(self, *, new_stars):
        ...
        if (
            self.stars
            < self.bot.guild_cache[self.guild_id]["starboard_star_requirement"]
        ):
    ...
    async def terminate(self):
        await self.bot.pool.execute(
            "DELETE FROM starboard_msgs WHERE message_id=$1", self.message_id
        )
...
class Starboard(commands.Cog):
    ...
    async def cog_check(self, ctx):
        if not ctx.guild:
            raise commands.CommandError("This category is restricted to guilds")
    ...
    @is_owner_or_administrator()
    async def set_starboard_config(
        self, ctx, setting, *, new_value: Union[discord.TextChannel, int]
    ):
        """
        ...
        await ctx.send(
            "Setting `{0}` successfully changed to `{1}`".format(setting, new_value)
        )
            ...
            stars = len(
                [
                    *filter(
                        ...
                        [
                            u
                            async for u in discord.utils.get(
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if (
            existing := discord.utils.get(
                filter(None, self.starred), message_id=payload.message_id
            )
        ) :
        ...
        if stars >= guild["starboard_star_requirement"]:
            ...
            if message.attachments:
                for attach in message.attachments:
                    embed.add_field(name="Attachment", value=f"[URL]({attach.url})")
                embed.set_image(url=message.attachments[-1].url)
            embed.add_field(
                name="Jump", value=f"[URL]({message.jump_url})", inline=False
            )
            try:
    ...
    async def on_raw_bulk_message_delete(self, payload):
        ...
        for msg_id in payload.message_ids:
            if (
                star := discord.utils.get(filter(None, self.starred), message_id=msg_id)
            ) :
                await star.terminate()
                self.starred.discard(star)
