...
from neo.utils.formatters import flatten, group
...
class Util(commands.Cog):
    ...
    async def imgur(self, ctx, *, image=None):
        ...
        data = {"image": image}
        async with ctx.loading(), self.bot.session.post(
    ...
    async def _ocr_command(self, ctx, *, image_url: str = None):
        ...
        async with timeout(30), ctx.loading(tick=False), self.bot.session.get(
            "https://api.tsu.sh/google/ocr", params={"q": image}
        ) as resp:
    ...
    async def random_choice_bestof(self, ctx, times: Optional[int], *options):
        ...
        formatted_choices = (
            f"{c}. `{choice.ljust(len(max(options, key=len)))}` "
            f"[{amt} | {amt/times * 100:.1f}%]"
            for c, (choice, amt) in enumerate(choices, 1)
        )
