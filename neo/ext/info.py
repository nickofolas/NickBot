...
statuses_base = namedtuple(
    "statuses_base", "online dnd idle offline", defaults=(0, 0, 0, 0)
)
...
class UserInfo:
            ...
            if isinstance(a, discord.Spotify):
                ...
                if a.emoji:
                    emoji = (
                        ...
                        and not self.ctx.bot.get_emoji(a.emoji.id)
                        else a.emoji
                    )
    ...
    def tagline(self):
        ...
        if self.user.bot:
            tagline += (
                "".join(info_emojis[f"veribot{i}"] for i in (1, 2))
                if "verified_bot" in self.flags
                else info_emojis["bot"]
            )
        if self.ctx.guild and isinstance(self.user, discord.Member):
...
class Info(commands.Cog):
    ...
    async def userinfo(self, ctx, *, target=None):
        ...
        flags = [
            flag for flag, value in dict(user.public_flags).items() if value is True
        ]
    ...
    async def perms(self, ctx, target: discord.Member = None):
            ...
            joined = "\n".join(
                [
                    ...
                    in [
                        a[0]
                        for a in filter(
                            lambda p: p[1] is True, discord.Permissions(2080898303)
                        )
                    ]
                ]
            )
        ...
        embed.set_author(
            name=target.display_name, icon_url=target.avatar_url_as(static_format="png")
        )
    ...
    async def spotify(self, ctx, target: discord.Member = None):
        ...
        if ac := discord.utils.find(
            lambda a: isinstance(a, discord.Spotify), target.activities
        ):
            ...
            e.set_author(
                name=textwrap.fill(ac.title[:256], width=42.5),
    ...
    async def rich_presence(self, ctx, target: discord.Member = None):
        ...
        if ac := discord.utils.find(
            ...
            if ac.start:
                elapsed = relativedelta(
                    seconds=(datetime.utcnow() - ac.start).total_seconds()
                ).normalized()
                elapsed_formatted = []
    ...
    async def serverinfo(self, ctx):
        ...
        statuses = statuses_base(
            **Counter([m.status.value for m in guild.members])
        )._asdict()
        s_members = [
            f'{neo.conf["emojis"]["status_emojis"][k]} {v:,}'
    ...
    def by_category_v2(g):
        ...
        top_non_cat = filter(
            (
                lambda c: (
                    (not c.category) and (not isinstance(c, discord.CategoryChannel))
                )
            ),
            g.channels,
        )
    ...
    def format_channels(channel):
        ...
        if isinstance(channel, discord.CategoryChannel):
            ...
            if isinstance(channel, discord.TextChannel) and channel.is_nsfw():
                suffix = "-nsfw"
            emoji = neo.conf["emojis"]["channel_indicators"][
                channel.__class__.__name__ + suffix
            ]
    ...
    async def roles(self, ctx):
        ...
        await ctx.paginate(
            list(reversed([r.mention for r in ctx.guild.roles[1:]])),
            20,
    ...
    async def _resolve_invite(self, ctx, *, guild_invite):
        ...
        async with ctx.loading():
            ...
            features_pprint = (
                ", ".join(
                    map(neo.utils.formatters.prettify_text, guild.features or ["None"])
                )
                .title()
                .replace("Url", "URL")
            )
