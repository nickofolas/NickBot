...
class Customisation(commands.Cog):
    ...
    async def user_settings(
        ...
        if setting_name is not None and new_setting is not None:
            ...
            if setting_name not in keys:
                raise commands.CommandError(
                    f"New setting must be one of {', '.join(keys)}"
                )
            async with ctx.loading():
                await self.bot.pool.execute(
                    f"UPDATE user_data SET {setting_name}=$1 WHERE user_id=$2",
                    new_setting,
                    ctx.author.id,
                )
        ...
        for k, v in self.bot.user_cache[ctx.author.id].items():
            ...
            if isinstance(v, bool):
                readable_settings.append(
                    f"{ctx.toggle(v)} **{discord.utils.escape_markdown(k)}**"
                )
            elif isinstance(v, (list, str)) or v is None:
                continue
            else:
                readable_settings.append(
                    f"**{discord.utils.escape_markdown(k)}** `{v}`"
                )
        embed.description = "\n".join(readable_settings)
        await ctx.send(
            embed=embed.set_thumbnail(url=ctx.author.avatar_url_as(static_format="png"))
        )

    @commands.command(name="config", aliases=["cfg"])
    @commands.guild_only()
    @is_owner_or_administrator()
    async def _guild_config(
        ...
        if setting_name is not None and new_setting is not None:
            ...
            async with ctx.loading():
                await self.bot.pool.execute(
                    f"UPDATE guild_prefs SET {setting_name}=$1 WHERE guild_id=$2",
                    new_setting,
                    ctx.guild.id,
                )
        ...
        embed.description = "\n".join(readable_settings)
        await ctx.send(
            embed=embed.set_thumbnail(url=ctx.guild.icon_url_as(static_format="png"))
        )
    ...
    async def _modify_guild_prefixes(self, ctx, prefix):
        ...
        async with ctx.loading():
            ...
            if strat == current_prefixes.discard and len(current_prefixes) == 1:
                raise commands.CommandError(
                    "A guild must always have at least one prefix"
                )
    ...
    async def highlight(self, ctx):
        ...
        my_hl = list(
            filter(
                lambda hl: hl.user_id == ctx.author.id, self.bot.get_cog("HlMon").cache
            )
        )
        await ctx.send(
            embed=discord.Embed(
    ...
    async def todo_rw(self, ctx):
        ...
        """
        todos = [
            shorten(r["f"], width=175)
            for r in await self.bot.pool.fetch(query, ctx.author.id)
        ]
        await ctx.paginate(
            todos,
            10,
    ...
    async def create_todo(self, ctx, *, content: str):
        ...
        """
        query = (
            "INSERT INTO todo (user_id, content, jump_url, created_at) "
            "VALUES ($1, $2, $3, $4) RETURNING content"
        )
        new = await self.bot.pool.fetchval(
            query, ctx.author.id, content, ctx.message.jump_url, datetime.utcnow()
        )
    ...
    async def remove_todo(self, ctx, todo_index: commands.Greedy[int]):
        ...
        if not todo_index:
            raise commands.CommandError(
                "Use the index of a todo [found in your list of todos] to remove it"
            )
        query = """
        WITH enumerated AS (
        ...
        await ctx.send(
            "Successfully deleted the following todos:\n{}".format(
                "\n".join(shown[:5]) + extra
            )
        )
    ...
    async def _remind(self, ctx, *, reminder: TimeConverter):
        ...
        await ctx.send(
            f"{ctx.tick(True)} Reminder set for {pretty_time} with ID `{reminder_id}`"
        )
    ...
    async def _remind_list(self, ctx):
        ...
        def format_reminder(reminder):
            time = reminder["deadline"].strftime(
                f"%a, %b %d, %Y at %H:%M:%S %Z ({naturaltime(reminder['deadline'])})"
            )
    ...
    def get_running_reminders(self):
        yield from filter(
            lambda task: task.get_name().startswith("REMINDER"),
            asyncio.all_tasks(self.bot.loop),
        )
    ...
    async def _remind_remove(self, ctx, items: commands.Greedy[int]):
        ...
        [
            task.cancel()
            for task in running
            if task.get_name().endswith(tuple(map(str, items)))
        ]
    ...
    async def _remind_clear(self, ctx):
        ...
        if confirm:
            ...
            [
                task.cancel()
                for task in self.get_running_reminders()
                if task.get_name().endswith((*map(lambda r: str(r["id"]), cancelled),))
            ]
